...
class UserEvaluationHistory(Base):
    ...
    user_id = Column(CHAR(36), ForeignKey("user.user_id"), nullable=False, comment="評価を行ったユーザーID (UUID)")
    
    # 評価されたユーザーID (UUID) - userテーブルのuser_idを参照する外部キー
    target_user_id = Column(CHAR(36), ForeignKey("user.user_id"), nullable=False, comment="評価されたユーザーID (UUID)")
